...
console.log("dadsad file loaded!");

document.addEventListener("DOMContentLoaded", () => {
    fetch('./api/getallproduct.php')
                ...
                for (let i = 0; i < product.length; i++) {
                    html += `<div onclick="openProductDetail(${i})" class='product-items ${product[i].type}'>
                                <img class='product-img'
                                    src="./img/${product[i].img}" alt="">
                                <p style="font-size: 1.2rem; padding-left: 1rem;">${product[i].name}</p>
            ...
            html += `<div class="cartlist-itemes">
                    <div class="cartlist-left">
                        ...
                        <div class="cartlist-detail">
                            <p style="font-size: 1.5rem;">${numberWithCommas(cart[i].name)}</p>
                            <p style="font-size: 1.2rem;">${numberWithCommas(cart[i].price * cart[i].count)} THB</p>
                        </div>
                    </div>
...
function Buynow() {
    $.ajax({
        ...
        url: './api/Buynow.php',
        data: {
            product: JSON.stringify(cart) // ส่งเป็น JSON string
        },
        success: function(response) {
            // ถ้า response เป็น string ให้แปลงเป็น object
